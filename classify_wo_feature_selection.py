...
with open('batch_results_stem.csv','rU') as csvfile:
    ...
    for row1,row2,row3 in grouper(reader, 3):
        ...
        if (sentiment1 == "Satisfied" and (sentiment2 == "Satisfied" or sentiment3 == "Satisfied")) or (sentiment2 == "Satisfied" and sentiment3 == "Satisfied"):
            sentiment = "Satisfied"
        p = (review_features(review),sentiment,review)
...
train_set, test_set = final_featuresets[(3*one_quarter):len_of_featuresets]+final_featuresets[one_quarter:]+final_featuresets[(one_quarter):(one_quarter*2)], final_featuresets[(one_quarter*2):(one_quarter*3)]

# classify
classifier = nltk.NaiveBayesClassifier.train([(fs,s) for fs,s,r in train_set])
...
for review_features,sentiment, _ in test_set:
...
# TODO: DEBUG
#errors = []
#for review_features, sentiment, review in test_set:
